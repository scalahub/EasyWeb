...
object MyConfig {
  ...
  val prefix =
    "myMethod_" // only methods starting with "myMethod_" will be considered
...
import org.sh.easyweb.MyConfig._
...
object MyProxy {
  ...
  List(mySecondClass).foreach(
    EasyProxy.addProcessor(prefix, _, DefaultTypeHandler, true)
  )
  List("*Restricted*").foreach(EasyProxy.preventMethod)
  DefaultTypeHandler.addType[MyType](
    classOf[MyType],
